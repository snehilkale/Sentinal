...
/* ---------------------------------------------------------------------
   Token system — instrument-panel palette (light + dark)
--------------------------------------------------------------------- */
const LIGHT = {
  bg: "#FFFFFF",
  panel: "#FFFFFF",
...
--------------------------------------------------------------------- */
function phaseColor(phase, C) { return C[phase] || C.muted; }

function RiskDial({ value, phase, C }) {
  const r = 54, c = 2 * Math.PI * r;
  const pct = Math.max(0, Math.min(1, value));
  const color = phase === "NORMAL" ? C.accent : phaseColor(phase, C);
  return (
    <svg width="140" height="140" viewBox="0 0 140 140" style={{ overflow: "visible" }}>
      ...
      <circle cx="70" cy="70" r={r} fill="none" stroke={C.line} strokeWidth="9" />
      <circle
      ...
      />
      <text x="70" y="66" textAnchor="middle" fontFamily="-apple-system, 'SF Pro Text', sans-serif"
        fontSize="28" fontWeight="500" fill={C.text}>{(pct * 100).toFixed(0)}</text>
      <text x="70" y="87" textAnchor="middle" fontFamily="'SF Mono', ui-monospace, monospace"
        fontSize="10" letterSpacing="1.5" fill={C.muted}>SENTINEL RISK</text>
    </svg>
  );
}
...
function Field({ label, value, unit, C }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
      <span style={{ fontSize: 10, letterSpacing: "0.06em", color: C.dim }}>{label}</span>
      <span style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 15, color: C.text }}>
        {value}{unit ? <span style={{ color: C.muted, fontSize: 11 }}> {unit}</span> : null}
      </span>
    </div>
  );
}
...
export default function SentinelConsole() {
  ...
  const mono = "'SF Mono', ui-monospace, 'IBM Plex Mono', monospace";

  return (
    <div style={{
      background: C.bg, color: C.text, fontFamily: font,
      padding: 24, borderRadius: 20, maxWidth: 980, margin: "0 auto",
      border: `1px solid ${C.line}`,
    }}>
      <style>{`
        .sentinel-btn {
          background: ${C.panel2}; color: ${C.text}; border: 1px solid ${C.line};
          font-family: ${mono}; font-size: 11px; padding: 7px 14px; cursor: pointer;
          letter-spacing: 0.04em; border-radius: 8px; transition: border-color 120ms, color 120ms, background 120ms;
        }
        .sentinel-btn:hover { border-color: ${C.accent}; color: ${C.accentStrong}; background: ${C.accent}14; }
        .sentinel-btn.active { border-color: ${C.accent}; color: ${C.accentStrong}; }
        .sentinel-select {
          background: ${C.panel2}; color: ${C.text}; border: 1px solid ${C.line};
          font-family: ${mono}; font-size: 11px; padding: 7px 10px; border-radius: 8px;
        }
        .sentinel-select:focus, .sentinel-btn:focus-visible {
          outline: none; border-color: ${C.accent};
          box-shadow: 0 0 0 2px ${C.accent}33;
        }
        input[type=range].sentinel-range {
          -webkit-appearance: none; width: 100%; height: 2px; background: ${C.line}; border-radius: 2px;
        }
        input[type=range].sentinel-range::-webkit-slider-thumb {
          -webkit-appearance: none; width: 12px; height: 12px; background: ${C.accent};
          border-radius: 50%; cursor: pointer; margin-top: -5px;
          box-shadow: 0 0 0 3px ${C.accent}22;
        }
        .sentinel-alert-row:hover { background: ${C.accent}0c; }
      `}</style>

      {/* ---------- header ---------- */}
      <div style={{
        display: "flex", justifyContent: "space-between", alignItems: "center",
        borderBottom: `1px solid ${C.line}`, paddingBottom: 16, marginBottom: 20, flexWrap: "wrap", gap: 10,
      }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <div style={{
            width: 34, height: 34, borderRadius: 10,
            background: `linear-gradient(150deg, ${C.accent}, #6E5BC7)`,
            display: "flex", alignItems: "center", justifyContent: "center",
            boxShadow: `0 0 0 1px ${C.line}, 0 4px 14px -4px ${C.accent}66`,
          }}>
            <svg width="17" height="17" viewBox="0 0 24 24" fill="none">
              <circle cx="12" cy="12" r="9" stroke="#0E0D13" strokeWidth="1.6" />
              <circle cx="12" cy="12" r="3" fill="#0E0D13" />
            </svg>
          </div>
          <div>
            <div style={{ fontSize: 14, letterSpacing: "0.1em", color: C.text, fontWeight: 500 }}>SENTINEL</div>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
          <span style={{ fontSize: 11, color: C.dim, fontFamily: mono }}>lead category</span>
          <select
      ...
      <div style={{ display: "flex", gap: 20, marginBottom: 20, flexWrap: "wrap" }}>
        <div style={{
          ...
          boxShadow: "0 8px 24px -14px rgba(60,40,110,0.18)",
        }}>
          <RiskDial value={cur.sentinelRisk} phase={cur.phase} C={C} />
          <div style={{
            fontFamily: mono, fontSize: 11, letterSpacing: "0.08em", color: phaseColor(cur.phase, C),
            border: `1px solid ${phaseColor(cur.phase, C)}`, padding: "4px 14px", borderRadius: 999,
            background: `${phaseColor(cur.phase, C)}14`,
          }}>{cur.phase}</div>
        </div>
        ...
        <div style={{
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(120px, 1fr))", gap: 14 }}>
            <Field label="STEP" value={`${cur.step} / ${n - 1}`} C={C} />
            <Field label="GROUND TRUTH RISK" value={cur.risk.toFixed(3)} C={C} />
            <Field label="FUSED RISK" value={cur.sentinelRisk.toFixed(3)} C={C} />
            <Field label="FCER" value={cur.fCER.toFixed(3)} C={C} />
            <Field label="IF ANOMALY" value={cur.ifScore.toFixed(3)} C={C} />
            <Field label="AE ANOMALY" value={cur.aeScore.toFixed(3)} C={C} />
          </div>
          <div>
      ...
      <div style={{ marginBottom: 20 }}>
        <div style={{ fontSize: 11, letterSpacing: "0.08em", color: C.dim, marginBottom: 8, fontFamily: mono }}>
          RISK TIMELINE
        </div>
        <div style={{
          background: `linear-gradient(165deg, ${C.panel2}, ${C.panel})`,
            ...
            <LineChart data={data} margin={{ top: 4, right: 8, bottom: 0, left: 0 }}
              ...
              {bands.map((b, i) => (
                <ReferenceArea key={i} x1={b.from} x2={b.to} y1={0} y2={1}
                  fill={phaseColor(b.phase, C)} fillOpacity={0.08} stroke="none" />
              ))}
              <ReferenceArea x1={cur.step} x2={cur.step} y1={0} y2={1} stroke={C.accent} strokeOpacity={0.8} />
              <Line type="monotone" dataKey="risk" stroke={C.muted} strokeWidth={1} dot={false} isAnimationActive={false} />
              <Line type="monotone" dataKey="sentinelRisk" stroke={C.EMERGENCY} strokeWidth={1.6} dot={false} isAnimationActive={false} />
              <Tooltip
          ...
          <div style={{ display: "flex", gap: 16, marginTop: 6, flexWrap: "wrap" }}>
            {PHASES.map((p) => (
              <div key={p} style={{ display: "flex", alignItems: "center", gap: 5 }}>
                <span style={{ width: 8, height: 8, background: phaseColor(p, C), display: "inline-block" }} />
                <span style={{ fontSize: 10, color: C.muted, fontFamily: mono }}>{p}</span>
              </div>
            ))}
            <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
              <span style={{ width: 12, height: 1.5, background: C.muted, display: "inline-block" }} />
              <span style={{ fontSize: 10, color: C.muted, fontFamily: mono }}>ground truth</span>
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
              <span style={{ width: 12, height: 1.5, background: C.EMERGENCY, display: "inline-block" }} />
              <span style={{ fontSize: 10, color: C.muted, fontFamily: mono }}>sentinel_risk</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ display: "flex", gap: 18, marginBottom: 20, flexWrap: "wrap" }}>
        <div style={{ flex: 2, minWidth: 320 }}>
          ...
          <div style={{
            ...
            {entropyChannels.map((c, i) => (
              <div key={c.key} style={{
                ...
                <div style={{ width: 150, fontSize: 10, color: C.muted, fontFamily: mono }}>{c.label}</div>
                <div style={{ flex: 1 }}>
                  <Trace data={data} dataKey={c.key} color={C.accent} height={40} />
                </div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 16px", background: `${C.accent}08` }}>
              <div style={{ width: 150, fontSize: 10, color: C.text, fontFamily: mono }}>fCER (fused)</div>
              <div style={{ flex: 1 }}>
                <Trace data={data} dataKey="fCER" color={cur.persistentAlert ? C.EMERGENCY : C.SUSPICIOUS} height={40} />
              </div>
              <div style={{ width: 44, textAlign: "right", fontFamily: mono, fontSize: 13, color: C.text }}>
                {cur.fCER.toFixed(2)}
              </div>
            </div>
          </div>
          {cur.persistentAlert && (
            <div style={{
              marginTop: 10, fontFamily: mono, fontSize: 11, color: C.EMERGENCY,
              border: `1px solid ${C.EMERGENCY}66`, padding: "7px 14px", borderRadius: 999,
              display: "inline-flex", alignItems: "center", gap: 6, background: `${C.EMERGENCY}12`,
            }}>
              <span style={{ width: 6, height: 6, borderRadius: "50%", background: C.EMERGENCY, display: "inline-block" }} />
        ...
        <div style={{ flex: 1, minWidth: 220 }}>
          ...
          <div style={{
            ...
            {[
              { key: "contribAnomaly", label: "anomaly (IF+AE)", color: "#B9A3FF" },
              { key: "contribPhase", label: "phase transition", color: "#8B7FD9" },
              { key: "contribEntropy", label: "entropy (fCER)", color: "#E8B339" },
              { key: "contribRules", label: "rules backstop", color: "#655F79" },
            ].map((row) => {
              ...
              return (
                <div key={row.key} style={{ marginBottom: 12 }}>
                  ...
                  <div style={{ height: 7, background: C.lineFaint, borderRadius: 999, overflow: "hidden" }}>
                    <div style={{ width: `${w}%`, height: "100%", background: row.color, borderRadius: 999, transition: "width 150ms ease-out" }} />
                  </div>
                </div>
              );
            })}
            <div style={{ borderTop: `1px solid ${C.line}`, marginTop: 10, paddingTop: 10, display: "flex", justifyContent: "space-between", fontFamily: mono, fontSize: 12 }}>
              <span style={{ color: C.muted }}>sentinel_risk</span>
              <span style={{ color: C.accentStrong }}>{cur.sentinelRisk.toFixed(3)}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div>
        ...
        <div style={{
          ...
          {alertEvents.length === 0 && (
            <div style={{ padding: 14, fontSize: 12, color: C.dim, fontFamily: mono }}>
          ...
          {alertEvents.map((ev, i) => (
            <div
              ...
              className="sentinel-alert-row"
              style={{
                display: "flex", justifyContent: "space-between", padding: "10px 16px", cursor: "pointer",
                borderBottom: i < alertEvents.length - 1 ? `1px solid ${C.lineFaint}` : "none",
                fontFamily: mono, fontSize: 12,
              }}
            >
              <span style={{ color: C.EMERGENCY }}>fCER persistent alert</span>
              <span style={{ color: C.muted }}>step {ev.step} · phase {ev.phase}</span>
            </div>
          ))}
        </div>
      </div>
